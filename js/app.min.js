...
function initStartScreen() {
    ...
    gameState = "start", gameScore = 0, userInput.addHitArea("mute", butEventHandler, null, {
        type: "rect",
        aRect: [336, 0, canvas.width, 48]
    }, !0), background = new Elements.Background(assetLib.getData("background"), canvas.width, canvas.height);
    var a = {
...
function initPreGame() {
    ...
    for (var a = 0; a < aTutorials.length; a++)
        if (aTutorials[a].gameNum == gameNum && !aTutorials[a].shown) {
            background = new Elements.Background(assetLib.getData("background"), canvas.width, canvas.height), userInput.addHitArea("continue", butEventHandler, null, {
                type: "rect",
...
function initGame() {
    gameState = "game", allowSound && music.volume(.5), background = new Elements.Background(assetLib.getData("background"), canvas.width, canvas.height), userInput.addHitArea("pause", butEventHandler, null, {
        type: "rect",
        aRect: [294, 0, 336, 48]
    }, !0), hud = new Elements.Hud(assetLib.getData("hud"), assetLib.getData("thinNumbers"), assetLib.getData("fatNumbers"), canvas.width, canvas.height), aDots = new Array, aSelected = new Array, gameTime = 60, gameScore = 0, blockTap = !1, longestChain = 0, balloonsPopped = 0, lastSelectedDot = null;
    for (var a = 0; 42 > a; a++) {
        var b = new Elements.Dot(assetLib.getData("dot"), assetLib.getData("pop"), a, getNewDotType(), canvas.width, canvas.height);
        b.targX = Math.round(62 * (a % 6) + 36), b.targY = Math.round(getYPosFromId(a)), b.fall(), userInput.addHitArea("dotHit", butEventHandler, {
            id: a,
            isDraggable: !0
        }, {
            type: "rect",
            aRect: [b.x - 22, b.targY - 22, b.x + 22, b.targY + 22]
        }), aDots.push(b)
    }
    background.startScroll(20), userInput.addHitArea("mouseUp", butEventHandler, {
        multiTouch: !0
    }, {
        type: "rect",
...
function toggleManualPause() {
    if (manualPause)
        ...
        userInput.addHitArea("quitFromPause", butEventHandler, null, {
            ...
            oImageData: b.oImgData,
            aCentrePos: b.aPos
        }), panel = new Elements.Panel(assetLib.getData("panels"), assetLib.getData("thinNumbers"), assetLib.getData("fatNumbers"), "pause", c, canvas.width, canvas.height), panel.render(ctx), userInput.addHitArea("pause", butEventHandler, null, {
            type: "rect",
            aRect: [294, 0, 336, 48]
        }, !0)
    }
...
! function (a) {
    var b = function () {
        ...
        return a.prototype.startScroll = function (a) {
            this.targY -= 5 * a, TweenLite.killTweensOf(this), TweenLite.to(this, 4, {
                y: this.targY,
                ease: "Quad.easeOut"
            })
            ...
            switch (this.panelType) {
                ...
                case "gameOver":
                    ...
                    for (var e = this.oScoreData.longestChain, f = 0; f < e.toString().length; f++) {
                        b = parseFloat(e.toString().charAt(f));
                        var c = b * this.oThinNumbersImgData.oData.spriteWidth % this.oThinNumbersImgData.img.width,
                            d = Math.floor(b / (this.oThinNumbersImgData.img.width / this.oThinNumbersImgData.oData.spriteWidth)) * this.oThinNumbersImgData.oData.spriteHeight;
...
! function (a) {
    var b = function (a) {
        ...
        return __extends(b, a), b.prototype.fall = function () {
            this.y = this.targY + 550, this.x = this.targX, TweenLite.to(this, .5, {
                y: this.targY,
                delay: .02 * this.id,
                ease: "Quad.easeOut"
            })
        }, b.prototype.replaceWithExistingDot = function (a) {
            this.y = a.targY, this.type = a.type, this.removeMe = !1, this.canHit = !0, this.x = this.targX, this.changeSpriteSheet(this.oDotImgData), this.frameInc = a.frameInc, this.unSelect(), TweenLite.to(this, .5, {
                y: this.targY,
                ease: "Quad.easeInOut"
            })
        }, b.prototype.replaceWithNewDot = function (a) {
            this.y = this.targY + 550, this.type = a, this.removeMe = !1, this.canHit = !0, this.x = this.targX, this.changeSpriteSheet(this.oDotImgData), this.frameInc = Math.ceil(20 * Math.random()), this.unSelect(), TweenLite.to(this, .5, {
                y: this.targY,
                ease: "Quad.easeOut"
            })
...
}), (allowSound = !0, sound = new Howl({
    urls: ["audio/sound.mp3"],
    sprite: {
        pop1: [0, 400],
        pop2: [500, 400],
        pop3: [1e3, 750],
...
var panel, hud, background, gameScore, gameNum = 0,
    ...
    panelFrame, aDots, lastSelectedDot, aSelected, blockTap = !1,
    removedDots, gameTime, longestChain, balloonsPopped, highScore = 0;
loadPreAssets();
